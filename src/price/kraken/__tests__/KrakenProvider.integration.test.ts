...
describe('KrakenProvider (integration)', () => {
  it('fetches real BTC price from Kraken (skip unless RUN_INTEGRATION=true)', async () => {
    if (!runIntegration) {
      console.warn(
  ...
  it('fetches EUR price from Kraken (skip unless RUN_INTEGRATION=true)', async () => {
    if (!runIntegration) {
      console.warn(
        'Skipping integration test - set RUN_INTEGRATION=true to enable'
      );
